...
![QR Grab icon](Assets/app-icon-256.png)
...
.\release\win-x64-framework\QRGrab.exe
```
...
- `release\win-x64-framework\QRGrab.exe` is small and uses the installed .NET Desktop Runtime.
- `release\win-x64-self-contained\QRGrab.exe` is a larger standalone exe for Windows x64.
